...
const scrollMoreVariants: Variants = {
  ...
  bounce: {
    y: [0, -18, 0],
    transition: { duration: 1.6, ease: 'easeInOut', loop: Infinity, },
  },
}
    ...
    <Box position="fixed" bottom="1em" right="3%" display={isMobile ? 'none' : 'block'}    >
      <AnimatePresence>
        {[ScrollDirection.Initial, ScrollDirection.Up].includes(scrollDirection) && (
          <motion.div initial="initial" animate={['hidden', 'bounce']} variants={scrollMoreVariants}          >
      ...
      <AnimatePresence>
        {scrollDirection === ScrollDirection.Down && (
          <motion.div
            ...
            whileHover={{ y: -50 }}
            style={{ writingMode: 'vertical-rl', position: 'fixed', right: '8%', bottom: '-8%', }}
          >
            <Text
              as="a"
              paddingY={3}
              ...
              _hover={{
                color: emailLine,
                _after: { backgroundColor: emailLine, opacity: 1, },
              }}
              position="relative"
              letterSpacing={3}
              display="flex"
